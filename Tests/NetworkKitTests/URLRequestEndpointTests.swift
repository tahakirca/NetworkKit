...
struct URLRequestEndpointTests {
    ...
    func urlConstruction() throws {
        let request = try URLRequest(TestEndpoint.simple, baseURL: baseURL, encoder: encoder)
        #expect(request.url?.absoluteString == "https://api.test.com/test")
    }

    @Test("appends query items")
    func queryItems() throws {
        let request = try URLRequest(TestEndpoint.withQuery(page: 3), baseURL: baseURL, encoder: encoder)
    ...
    func httpMethod() throws {
        let get = try URLRequest(TestEndpoint.simple, baseURL: baseURL, encoder: encoder)
        let post = try URLRequest(TestEndpoint.withJSON(TestBody(name: "a", age: 1)), baseURL: baseURL, encoder: encoder)
    ...
    func customHeaders() throws {
        let request = try URLRequest(TestEndpoint.withHeaders, baseURL: baseURL, encoder: encoder)
    ...
    func jsonBody() throws {
        ...
        let request = try URLRequest(TestEndpoint.withJSON(body), baseURL: baseURL, encoder: encoder)
    ...
    func formBody() throws {
        let request = try URLRequest(TestEndpoint.withForm(["key": "val"]), baseURL: baseURL, encoder: encoder)
    ...
    func dataBody() throws {
        ...
        let request = try URLRequest(TestEndpoint.withData(data), baseURL: baseURL, encoder: encoder)
